...
function ENT:Initialize()
	...
	timer.Create( tostring(self.Entity), 100, 0, self.giveMoney, self)
	timer.Create( tostring(self.Entity) .. "fuckafkfags", 1200, 1, self.shutOff, self)
	timer.Create( tostring(self.Entity) .. "notifyoff", 1000, 1, self.notifypl, self)
...
function ENT:Reload()
	...
	timer.Create( tostring(self.Entity) .. "fuckafkfags", 500, 1, self.shutOff, self)
	timer.Destroy( tostring(self.Entity) .. "notifyoff")
	timer.Create( tostring(self.Entity) .. "notifyoff", 400, 1, self.notifypl, self)
	self.Inactive = false
	self.NearInact = false
