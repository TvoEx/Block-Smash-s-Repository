...
function ENT:Use( activator )
	...
	umsg.Start( "testkillgunfactorygui", activator );
		umsg.Short( self.Entity:EntIndex() )
	umsg.End()
	umsg.Start( "testgunfactorygui", activator );
		umsg.Short( self.Entity:GetNWInt("upgrade") )
		umsg.Short( self.Entity:EntIndex() )
		umsg.Bool(false)
	umsg.End()
...
function ENT:CanProduce(guntype, ply)
	...
	elseif !self.Entity:IsPowered() || !self.Ready then
		return false
	elseif guntype=="glock18" then
		return true
	elseif guntype=="deagle" then
		return true
	elseif guntype=="m4a1" && self.Entity:GetNWInt("upgrade")>=1 then
		return true
	elseif guntype=="mac10" && self.Entity:GetNWInt("upgrade")>=1 then
		return true
	elseif guntype=="rocketlauncher" && self.Entity:GetNWInt("upgrade")>=1 then
		return true
	elseif guntype=="para" && self.Entity:GetNWInt("upgrade")>=2 then
		return true
	else
		return true
	end
...
function ENT:StartProduction(ply,guntype)
	local time = 120
	if guntype=="m4a1" then time=280 elseif guntype=="para" then time = 340 elseif guntype=="mac10" then time = 180 elseif guntype=="rocketlauncher" then time = 200  end
	self.User = ply
	...
	umsg.Start( "testkillgunfactorygui", ply );
		umsg.Short( self.Entity:EntIndex() )
	umsg.End()
end
...
function ENT:DoStuff()
	...
	if !owner:CanAfford(productioncost) then
		...
		timer.Destroy(tostring(self.Entity) .. "spawned_weapon")
		self.Ready = true
		umsg.Start("testgunfactoryget",ply)
			umsg.Float(time+CurTime())
			umsg.Entity(self.Entity)
		umsg.End()
		return
...
function ENT:createGun()
	...
	if self.GunType=="resetbutton" then
		return
	elseif self.GunType=="deagle" then
		gun:SetModel("models/weapons/w_pist_deagle.mdl")
		gun:SetNWString("weaponclass","weapon_deagle2")
	elseif self.GunType=="glock18" then
		gun:SetModel("models/weapons/w_pist_glock18.mdl")
		gun:SetNWString("weaponclass","weapon_glock2")
	elseif self.GunType=="para" then
		gun:SetModel("models/weapons/w_mach_m249para.mdl")
		gun:SetNWString("weaponclass","weapon_para2")
	elseif self.GunType=="m4a1" then
		gun:SetModel("models/weapons/w_rif_m4a1.mdl")
		gun:SetNWString("weaponclass","weapon_m42")
	elseif self.GunType=="mac10" then
		gun:SetModel("models/weapons/w_smg_mac10.mdl")
		gun:SetNWString("weaponclass","weapon_mac102")
	end
	local trace = { }
